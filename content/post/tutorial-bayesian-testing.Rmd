...
To test these hypotheses, we can write them out in a single string and use the `confirm` function. Note that hypotheses are separated by a semicolon, and that partial correlations are denoted as `node1 -- node2`. The output is obtained by simply printing out the resuls of `confirm`.
