...
output: 
  html_document:
    ...
    toc_depth: 4
draft: true
bibliography: [C:/Users/josue/Box/formalizing-expectations/references.bib]
csl: apa.csl
---
...
The next step is to calculate bridge strength using the `networktools` package. This is similar to node strength, in that, for a given node, it is the sum of the absolute values of its edges. However, bridge strength only takes into account edges that connect a node to different communities, or clusters. Thus, it is a measure of inter-community connectivity, and can be used to identify central structures in a network. 
...
The output includes both the posterior probabilities and all the Bayes factors. The Bayes factors are in reference to the rows relative to the columns. For example the element in the 2nd row and 1st column would be interpreted as $\text{BF}_{21} = 4.05$
